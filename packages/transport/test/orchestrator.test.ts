...
import { startAllTransports, stopAllTransports, TransportOrchestrationError } from '../src/orchestrator';
...
describe('Transport Orchestrator', () => {
  ...
  describe('startAllTransports', () => {
    ...
    it('should throw error for null registry', async () => {
      await expect(startAllTransports(null as any, mockDispatch)).rejects.toThrow('Registry cannot be null or undefined');
    });

    it('should throw error for undefined registry', async () => {
      await expect(startAllTransports(undefined as any, mockDispatch)).rejects.toThrow('Registry cannot be null or undefined');
    });

    it('should throw error for non-function dispatch', async () => {
      await expect(startAllTransports(registry, 'not-a-function' as any)).rejects.toThrow('Dispatch must be a function');
    });
    ...
    it('should collect and report transport start failures', async () => {
      ...
      try {
        ...
        expect(orchestrationError.message).toContain('Failed to start 2 of 4 transports (2 succeeded)');
    ...
    it('should handle all transports failing', async () => {
      ...
      try {
        ...
        expect(orchestrationError.message).toContain('Failed to start 2 of 2 transports (0 succeeded)');
        expect(orchestrationError.failures).toHaveLength(2);
      }
    });
  ...
  describe('stopAllTransports', () => {
    ...
    it('should throw error for null registry', async () => {
      await expect(stopAllTransports(null as any)).rejects.toThrow('Registry cannot be null or undefined');
    });

    it('should throw error for undefined registry', async () => {
      await expect(stopAllTransports(undefined as any)).rejects.toThrow('Registry cannot be null or undefined');
    });
    ...
    it('should collect and report transport stop failures', async () => {
      ...
      try {
        ...
        expect(orchestrationError.message).toContain('Failed to stop 2 of 4 transports (2 succeeded)');
        expect(orchestrationError.failures).toHaveLength(2);
